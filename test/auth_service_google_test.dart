...
class FakeUserRepository implements UserRepository {
  ...
  Future<Result<void>> createUser(UserModel user) async {
    if (shouldFailCreation) {
      return const Failure('Firestore creation failed', AuthException('DB error'));
    }
    users[user.uid] = user;
...
void main() {
  group('AuthService Google Sign-In Unit Tests', () {
    ...
    test('FakeUserRepository checkUserExists returns correct boolean', () async {
      ...
      expect((fetchedUser as Success<UserModel>).data.email, equals('google@example.com'));
    });

    test('FakeUserRepository functions as expected for Google Sign-In persistence', () async {
