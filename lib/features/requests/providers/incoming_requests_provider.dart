...
final incomingRequestsProvider = StreamProvider.autoDispose<List<IncomingRequestData>>((ref) async* {
  ...
  final requestsStream = requestRepo.streamRequestsForOwner(currentUser.uid);
    ...
    for (final req in requests) {
      ...
      if (passenger == null) {
        ...
        if (result is Success<UserModel>) {
          ...
          passenger = UserModel.empty().copyWith(uid: req.requesterUid, name: 'Unknown');
        }
      }
      ...
      populatedRequests.add(IncomingRequestData(
        request: req,
        ride: ride,
        passenger: passenger,
      ));
