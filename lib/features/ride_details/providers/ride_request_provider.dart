...
final liveRideProvider = StreamProvider.autoDispose.family<RideModel, RideModel>((ref, initialRide) {
  final repo = ref.watch(rideRepositoryProvider);
  return repo.streamRide(initialRide.id).map((event) => event ?? initialRide);
});

final liveRideRequestsProvider = StreamProvider.autoDispose.family<List<RideRequestModel>, String>((ref, rideId) {
  final repo = ref.watch(rideRequestRepositoryProvider);
